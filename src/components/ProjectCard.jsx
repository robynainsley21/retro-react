import React, { useState } from "react";

const ProjectCard = ({ imageUrl, title, description, live, github, key }) => {
  const [isHovered, setIsHovered] = useState(false);

  return (
    <div className="mb-3 m-auto" key={key}>

      <div
        className="relative m-auto overflow-hidden cursor-pointer rounded-lg shadow-lg project-card m-auto"
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
        style={{
          transition: "all 0.3s ease-in-out",
        }}
      >
        <img
          src={imageUrl}
          alt={title}
          loading="lazy"
          className="w-full h-full object-cover project-image"
          style={{
            transition: "transform 0.3s ease-in-out",
            transform: isHovered ? "scale(1.1)" : "scale(1)",
          }}
        />
        <h4 className="mt-3 font-bold mb-2 text-center">{title}</h4>

        <div
          className="absolute inset-0 bg-black bg-opacity-70 flex flex-col justify-center items-center p-2 project-description"
          style={{
            transition: "opacity 0.3s ease-in-out, transform 0.3s ease-in-out",
            opacity: isHovered ? 0.9 : 0,
            transform: isHovered ? "translateY(-95%)" : "translateY(100%)",
          }}
        >
          <p className="text-white text-sm text-center">{description}</p>
        </div>
        
      </div>
      <div className="project-btns">
        <a href={live} className="github-btn" target="_blank">
          <svg
            width="40"
            height="40"
            xmlns="http://www.w3.org/2000/svg"
            fill="currentColor"
            className="bi bi-laptop"
            viewBox="0 0 16 16"
          >
            <path d="M13.5 3a.5.5 0 0 1 .5.5V11H2V3.5a.5.5 0 0 1 .5-.5zm-11-1A1.5 1.5 0 0 0 1 3.5V12h14V3.5A1.5 1.5 0 0 0 13.5 2zM0 12.5h16a1.5 1.5 0 0 1-1.5 1.5h-13A1.5 1.5 0 0 1 0 12.5" />
          </svg>
        </a>
        <a href={github} className="github-btn" target="_blank">
          <svg
            width="40"
            height="40"
            xmlns="http://www.w3.org/2000/svg"
            data-name="Layer 1"
            viewBox="0 0 24 24"
            id="github"
          >
            <path d="M12,2.2467A10.00042,10.00042,0,0,0,8.83752,21.73419c.5.08752.6875-.21247.6875-.475,0-.23749-.01251-1.025-.01251-1.86249C7,19.85919,6.35,18.78423,6.15,18.22173A3.636,3.636,0,0,0,5.125,16.8092c-.35-.1875-.85-.65-.01251-.66248A2.00117,2.00117,0,0,1,6.65,17.17169a2.13742,2.13742,0,0,0,2.91248.825A2.10376,2.10376,0,0,1,10.2,16.65923c-2.225-.25-4.55-1.11254-4.55-4.9375a3.89187,3.89187,0,0,1,1.025-2.6875,3.59373,3.59373,0,0,1,.1-2.65s.83747-.26251,2.75,1.025a9.42747,9.42747,0,0,1,5,0c1.91248-1.3,2.75-1.025,2.75-1.025a3.59323,3.59323,0,0,1,.1,2.65,3.869,3.869,0,0,1,1.025,2.6875c0,3.83747-2.33752,4.6875-4.5625,4.9375a2.36814,2.36814,0,0,1,.675,1.85c0,1.33752-.01251,2.41248-.01251,2.75,0,.26251.1875.575.6875.475A10.0053,10.0053,0,0,0,12,2.2467Z"></path>
          </svg>
        </a>
      </div>
    </div>
  );
};

export default ProjectCard;
